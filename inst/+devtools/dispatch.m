...
function T = toolTable (surface, sb)
  ...
  t.description = strcat ("Call one Octave function by name on typed", ...
    ...
    " no code text. A matrix is a list of rows; a range carries each cell's", ...
    " kind and value. Dates and times need datatypes. For programs: the", ...
    " values are in structuredContent only.");
  props = struct ();
  ...
  props.args = struct ("type", "array", "items", struct ("type", "object"), ...
    "description", ["Arguments in call order: number, string, logical,", ...
                    " matrix (a list of rows) or range"]);
...
function t = instructionsText (surface, sb)
  ...
  ## strcat strips trailing whitespace, so every separating space leads the
  ## chunk after it
...
%!test
%! ## Sentences stay apart: strcat strips the space a chunk ends with.
%! RESP = devtools.dispatch (mkreq ("server/discover", ""), []);
%! s = RESP.result.instructions;
